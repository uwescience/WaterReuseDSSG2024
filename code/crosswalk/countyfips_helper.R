...
#' helper function for county fips code
#' @description The goal of this task is to check whether county_fips is the identifier, correct it if not, and construct one from constituent parts.
#' For more information, visit the [census website](https://www.census.gov/programs-surveys/geography/guidance/geo-identifiers.html#:~:text=The%20full%20GEOID%20for%20many,codes%2C%20in%20which%20they%20nest).
...
#' @param cfips character vector. county_fips id column. 
#' @param state state id. constructor piece for county fips code. 
#' @param county county id. second constructor piece for county fips code.
#' @output cfips character vector named "cfips" with values all of length 5
#' @examples
