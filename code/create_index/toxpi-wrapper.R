...
check_id <- function(data, id_var) {
  ...
  #' 
  #' @param data dataframe the user wants to create a toxpiR index from
  #' @param id_var string. name of the column in data that identifies the basic unit over which the index is contstructed; 
  #' for example, in an index of drought risk over US counties
  #' the index might be 'county_id'
  #' @returns output type boolean - is the id valid (is it in the dataframe)?
...
create_index <- function(df, id_col, weights, index_name = "index_value") {
  ...
  #'
  #' @param low_color, high_color, na_color (strings) - the colors designating
