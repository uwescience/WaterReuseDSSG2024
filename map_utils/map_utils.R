...
#' Check the compatibility of the data and shapefile
#'
#' @param data The dataset that contains the column of interest
#' @param shapefile A shapefile with FIPS code
#' @param data_key,shape_key data_key is a column in data & shape_key is a column in shapefile data_key and shape_key have the same type
#'
#' @return A boolean that is true if this combination of data, shapefile, keys can be spatially joined
#' @export
#'
#' @examples
...
# helper to map_chloropleth()
#' Plot raw values to map 
#'
#' @param data expects a dataframe containing a geometry and a variable of interest. 
#' Usually, this dataframe will result from merging (1) a raw data file, 
#' indexed by a geographic region identifier like FIPS, 
#' with (2) a shapefile that specifies the geographic regions in question
#' @param variable a string containing the NAME of the column in data that contains the variable that should be used to make the map
#' @param caption a string defining the caption that appears  at the bottom right of the map figure
#' @param title the map title
#' @param low_color,high_color,na_color  the colors desigating the lowest and highest values of variable, respectively, 
#' and the color that should indicate missing data
#' @param map_font A string of the font type tp use in the title, caption, legends
#'
#' @return a ggplot2 object mapping the shapes defined in the dataset and
...
#' Plot percentiles
#'
#' @param data data (dataframe) - expects a dataframe containing a geometry
#' and a variable of interest. Usually, this dataframe will result
#' from merging (1) a raw data file, indexed by a geographic region
#' identifier like FIPS, with (2) a shapefile that specifies the
#' geographic regions in question

#' @param variable a string containing the NAME of the column in
#' data that contains the variable that should be used to make the map
#' @param caption a string defining the caption that appears
#' at the bottom right of the map figure
#' @param title A string of the map title
#' @param low_color,high_color,na_color low_color, high_color, na_color (strings) - the colors desigating
#' the lowest and highest values of variable, respectively, and the
#' color that should indicate missing data 
#' @param map_font font in the title, caption, legends
#'
#' @return ggplot2 object mapping the shapes defined in the dataset and
...
#' Plot choropleth shapes
#'
#' @param data expects a dataframe containing a variable of
#' interest, indexed by a geographic region identifier like FIPS
#' @param shapefile a shapefile that specifies the
#' geographic regions in question; must contain a 'geometry' column
#' @param data_key a string that contains the NAME of column in
#' data that should be used to merge it with the shapefile. This
#' should be something like "FIPS"
#' @param shape_key a string that contains the NAME of column in
#' the shapefile that should be used to merge it with the data. This
#' should be something like "FIPS"
#' @param variable a string containing the NAME of the column in
#' data that contains the variable that should be used to make the map
#' @param map_title the map title
#' @param map_caption a string defining the caption that appears
#' at the bottom right of the map figure
#' @param map_percentile A boolean of whether the map should display raw values
...
#' @param low_color,high_color,na_color the colors designating
#' the lowest and highest values of variable, respectively, and the
#' color that should indicate missing data
#' @param map_font A string of the font type in the title, caption, legends
#'
#' @return a ggplot2 object mapping the shapes defined in the dataset and
# color-coding them according to the value of variable; instead of
# plotting the RAW value of variable, this function coverts them
